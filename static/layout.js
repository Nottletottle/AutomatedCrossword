  ...
  for(let i = 0; i < rows; i++){
    for(let j = 0; j < cols - word.length + 1; j++){
      ...
      if(isValid && atleastOne && word.length > 1){
        var tempScore1 = computeScore1(connections, word);//connections
        var tempScore2 = computeScore2(rows, cols, i, j + (word.length / 2), word);//distance from center
        var tempScore3 = computeScore3(1, 0, verticalCount, totalCount);//vertical vs horizontal
        var tempScore4 = computeScore4(rows, word);//word length
